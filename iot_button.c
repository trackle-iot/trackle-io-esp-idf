...
static button_dev_t *button_create_com(uint8_t id, uint8_t active_level, uint8_t (*hal_get_key_state)(void *hardware_data), void *hardware_data, uint16_t long_press_ticks, uint16_t short_press_ticks)
{
    ...
    ESP_LOGI(TAG, "g_is_timer_running: %d", g_is_timer_running);
...
{
    ESP_LOGI(TAG, "IoT Button Version: %d.%d.%d", 1, 0, 0);
    ...
    case BUTTON_TYPE_GPIO:
    {
        ...
        btn = button_create_com(config->id, cfg->active_level, button_gpio_get_key_level, (void *)cfg->gpio_num, long_press_time, short_press_time);
    }
    break;
    case BUTTON_TYPE_CUSTOM:
    {
        ...
        btn = button_create_com(config->id,
                                config->custom_button_config.active_level,
